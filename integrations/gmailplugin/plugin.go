...
func (t *gmailListTool) Call(args map[string]any) (string, error) {
	...
	for i, email := range resp.Emails {
		...
		mdContent, err := htmlToMarkdown(email.Content)
		if err != nil {
			return "", err
		}
		emails[i] = fmt.Sprintf("# Email from %v to %s: %s\n%s", email.From, strings.Join(recip, ", "), email.Subject, mdContent)
	}
